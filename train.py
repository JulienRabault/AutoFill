...
from model.VAE.grid_trainer import grid_search as train_grid_vae

from model.trainer import train as train_pairvae
from model.grid_pair_trainer import grid_search as train_grid_pairvae
...
def parse_args():
    ...
    parser.add_argument("--gridsearch", type=str, default="off",choices = ["on","off"])
...
def main():
    ...
    if args.gridsearch == "on":
        if args.mode == "pair_vae" :
            train_grid_pairvae(config)
        elif args.mode == "vae" :
            train_grid_vae(config)
        else :
