...
class Conv1DVAE_concat(BaseVAE):
    def __init__(self, input_dim, latent_dim, learning_rate=1e-3):
        super(Conv1DVAE_concat, self).__init__(learning_rate=learning_rate)
        self.input_dim = input_dim
        self.latent_dim = latent_dim

        # Encoder
        self.encoder = nn.Sequential(
            nn.Conv1d(1, 32, kernel_size=3, stride=2, padding=1),  # (bs, 32, input_dim/2)
            nn.ReLU(),
            nn.Conv1d(32, 64, kernel_size=3, stride=2, padding=1),  # (bs, 64, input_dim/4)
            nn.ReLU(),
            nn.Flatten()
        )
        ...
        self.decoder = nn.Sequential(
            nn.ConvTranspose1d(64, 32, kernel_size=3, stride=2, padding=1),  # (bs, 32, input_dim/2)
            nn.ReLU(),
            nn.ConvTranspose1d(32, 1, kernel_size=3, stride=2, padding=1),  # (bs, 1, input_dim)
            nn.Sigmoid()
        )
